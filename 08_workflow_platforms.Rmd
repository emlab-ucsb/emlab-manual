...
3. GRIT should send you a confirmation email that your account has been created and that you have been added to our user group. Follow any additional instructions provided by GRIT to finish getting your account set up.

**If you already have a GRIT account but are not connected to the emLab group**

1. Send a help ticket email to GRIT (help@grit.ucsb.edu). Say that you are part of the emLab research group, which has Christopher Costello as the faculty PI. Ask them to please connect the email to the emLab group membership (for accessing emLab data storage space and emLab computational servers, including quebracho and sequoia) and Nextcloud group membership. 

2. GRIT should send you a confirmation email that you have been added to our user group. Follow any additional instructions provided by GRIT to finish getting your account set up.
...
**Step 3:** When prompted, enter https://emlab-nextcloud.grit.ucsb.edu as the server address.
...
![](images/nextcloud_browser_login.png)
...
- Cons
    ...
    - You will not be able to view the full emLab directory structure on your local machine. However, you can always view the entire directory structure using the Nextcloud browser interface: [https://emlab-nextcloud.grit.ucsb.edu](https://emlab-nextcloud.grit.ucsb.edu)
...
##### File option 2: Syncing the entire emLab directory using virtual files
...
The Windows version of the Nextcloud desktop app is the easiest to configure for virtual files, since virtual files are enabled by default. After logging in, you will be brought back to the Nextcloud app. Simply ensure that the virtual file option is selection ("Use virtual files instead of downloading content immediately"), and then click "Connect."

![](images/nextcloud_windows_virtual_files.png)

*Macs and Linux*: 

The Mac and Linux versions of the Nextcloud desktop app do not have virtual files enabled by default. You will need to enable this feature manually by going through the following steps.
...
**Step 2:** Before continuing, you now need to to set up virtual file support for your local machine, follow these instructions, which are different for Mac and Linux:

*Mac:*

Navigate to the folder **~/Library/Preferences/Nextcloud**. You can do this through the Finder, but a faster way to do this is as follows:
...
**Step 3:** Go back to the Nextcloud app by clicking on the Nextcloud icon in the upper bar of your screen. Click on your name in the app in the upper-right-hand (upper-left for Linux) corner of your desktop, click on your name, and then click Settings.
...
You may also see a warning message that you are about to enable virtual files. This is currently an experimental feature in Nextcloud, but we have been told by GRIT that it is safe to use. Click "enable experimental placeholder mode" to enable virtual files.
